...
#            x_eval = x_eval.flatten()
            y_train = [self.interp_function(self.coords[tuple(idx)]) for idx in idx_train] #k * x_eval[:, 0] #np.array()
            y_train = np.asarray(y_train).reshape((len(y_train), 1))#  np.squeeze()
...
#            print(y_eval)
            ...
            print(epoch, ' loss on train: ', current_loss, ',', ' loss on val (after weight adjustment): ', val_loss)
