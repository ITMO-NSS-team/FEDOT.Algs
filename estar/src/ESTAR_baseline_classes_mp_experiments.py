...
class Baseline_fitness(Specific_Operator):
    ...
    def apply(self, equation):
        ...
        rl_error = np.linalg.norm(np.dot(equation.features, weights[:-1]) + np.full(equation.target.shape, weights[-1]) - equation.target, ord = 2) # + self.alpha * np.linalg.norm(self.weights, ord = 1)) 
        if rl_error == 0:
            equation.fitness_value = np.inf
        else:
            equation.fitness_value = 1 / (rl_error)
