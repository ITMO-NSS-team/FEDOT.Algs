...
def check_dominance(target, compared_with) -> bool:
    return (np.all([target.obj_fun[obj_fun_idx] <= compared_with.obj_fun[obj_fun_idx] for obj_fun_idx in np.arange(target.obj_fun.size)]) and 
            np.any([target.obj_fun[obj_fun_idx] < compared_with.obj_fun[obj_fun_idx] for obj_fun_idx in np.arange(target.obj_fun.size)]))
...
def fast_non_dominated_sorting(population) -> list:
    ...
    while len(current_level_idxs) > 0:
        ...
        level_idx += 1
        current_level_idxs = deepcopy(new_level_idxs)
    return levels
